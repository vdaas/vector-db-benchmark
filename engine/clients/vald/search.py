...
from engine.clients.vald.config import SERVICE
...
class ValdSearcher(BaseSearcher):
    ...
    def init_client(
        ...
        channel = grpc.insecure_channel(f"{host}:{SERVICE.spec}", grpc_opts)
    ...
    def search_one(
        ...
        res = cls.stub.Search(
            payload_pb2.Search.Request(
                vector=vector,
                config=payload_pb2.Search.Config({**cls.cfg, **{"num": top}}),
            )
        )
        return [(int(r.id), r.distance) for r in res.result]
